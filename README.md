...
**仿 Scratch 的分类积木区 + 运行高亮 + 控制台**：左侧分类导航（彩色圆点，点击/滚动联动），
右侧**「所见即所得」积木**——调色板里的积木与画布上长得完全一致（同一渲染器）；实时预览
**执行到哪个积木，画布上那块就发光**；舞台页带**控制台**，`print` 实时输出、可一键打印积木信息：

![仿 Scratch 分类积木区](docs/images/ide_palette.png)
![舞台控制台](docs/images/ide_console.png)
