...
**实时编辑预览（类 Vue）**：右下角内置一个**积木解释器**，直接在浏览器里跑当前精灵的程序——编辑积木/文本即重新运行，支持键盘事件、广播、声音（Web Audio）。调色板新增**事件 / 声音**类积木（按键、广播、收到、播放音调）。下图：左为积木、右上为可编辑源码、右下为正在运行的小游戏（按住 → 精灵已右移）：

![实时预览](docs/images/ide_preview.png)
