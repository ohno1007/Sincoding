...
> **实施进展（实读代码后调整）**：M1（递归 TypeRef 纯重构）不单独做——实读发现聚合数据
> 用现有扁平三元组 `{base,len,structName}`（含单层嵌套）即可实现，**不需要递归对象**；
> 递归 TypeRef 留到真正必需的场景（数组的数组、结构体数组字段）再引入。已按此完成 M2 主体：
>
> - ✅ **数组作参数/返回值/整体赋值**（D2 值语义，C 侧 `struct { T data[N]; }` 包裹）
> - ✅ **结构体数组** `Enemy[3]`（下标读写、作参数），修了元素赋值的类型漏洞
> - ✅ **嵌套结构体字段** `Enemy{pos: Point, trail: float[3]}`（链式访问、顺序声明防环）
> - ✅ **guardian 用正经结构重写**：`Faller` 结构体数组取代 4 个平行全局数组，端到端编译
>   成 raylib 原生并无头跑通（M6 主体达成）
> - ✅ 环境打通：raylib 6.0.0 + emscripten 6.0.5；`sinc.wasm` 已重编；`run_tests` 全绿(54/0)
>
> **未做（留待需要时）**：结构体数组字段 `World{es: Enemy[8]}`（需 typedef 拓扑排序）、
> 数组的数组 `int[3][4]`（需递归 TypeRef）、嵌套左值 `arr[i].field = v`（现用「取出→改→放回」替代）。
...
> **M4 进展**：模块系统与内置标准库已落地 ——
> `import "模块"`（内置 std → 同目录 → `SINCODING_PATH`）、嵌套导入、循环检测、
> extern 原型去重；`std/mathx.sin` **用 Sincoding 自己写**并在构建期嵌入编译器，
> 故浏览器内（无文件系统）同样可 import；`--emit src`/积木视图只写回 import 行，
> 库源码不污染用户文件。
> **已完成（导入即得积木）**：编辑器按被导入函数的**签名**自动生成调色板分类——
> void 函数→语句块、有返回值→reporter，参数名/类型决定槽位；签名一变积木自动跟着变，
> 不读任何外观元数据（元数据永不驱动语义）。浏览器验证见 tools/verify_lib_palette.js。
> **未做**：`.sinlib` 库包格式（zip 分发）、IDE 库管理面板、库积木的中文显示名。
> **已解决（切片 T[]）**：上一轮识别的阻塞——「数组长度属于类型，通用数组库无法用一份
> 代码覆盖各长度」——已用**切片**解决（比泛型轻得多）：`T[]` 是借用视图（元素指针+长度），
